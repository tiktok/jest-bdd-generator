...
- In this Demo, the **`test.each(..)('Float and double', ...);`** implemented without step comments
  - Click the second button at the bottom:
...
The left side is a testing has **only one** `test()` marked with gherkin comments

Whereas the **two** `test` have similar or repeating code statements.

<TestGeneration testsSource={tests} />
