  ...
  const loser: React.FormEventHandler = (e): void => {
    ...
    if (target.value === '') {
      ...
      const value = Array.isArray(selections)
        ...
        : convertStringyValue(selections, target.value as typeof selections); // 1.23e10
      data[name] = value;
      setExpression((value !== undefined ? target.value : '') as T[keyof T]);
    }
  ...
  return (
    ...
    >
      ...
      {(displayTitle.length === 0 ? [undefined] : displayTitle).map((exp, i) => {
        const value = exp ? colHead.expression[exp] : undefined;
        let display: string;
        if (value === undefined) {
          display = '-empty-';
          ...
          await Promise.all(
            c.map(async (colExp) => {
              try {
                ...
                return execute(options).catch((err) => <s title={err.toString()}>Error</s>);
              } catch (err: any) {
