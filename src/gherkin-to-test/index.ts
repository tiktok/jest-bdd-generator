...
export class TestGenerator extends TestGeneratorFromSource {
  ...
  generateGherkin(options: { pathTestInput: string; pathGherkinInput: string }): string | undefined {
    const { pathTestInput, pathGherkinInput } = options;
    const steps = this.compileKnownSteps(pathTestInput);
    const gherkinSource = fs.readFileSync(pathGherkinInput).toString();
    return this.generateGherkinFromSource(steps, gherkinSource);
  }
}
