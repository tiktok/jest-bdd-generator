...
export class TestGeneratorFromSource {
  ...
  compileKnownStepsFromSource(source: string): JestToGherkin['output'] {
    ...
    const o = _piler.transpile(source, { fileName: 'pathTestInput.test.ts' }).outputText;
        ...
        const sourceSteps = p.scenario?.steps
          .map((pStep) => {
            ...
            if (sourceInOtherScenario) {
              return [
                `/** @${keyword}${pStep.text}`,
                `*     FROM: (${sourceInOtherScenario.parent}): [${sourceInOtherScenario.key} ${sourceInOtherScenario.value}]`,
